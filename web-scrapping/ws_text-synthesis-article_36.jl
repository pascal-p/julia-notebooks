...
ref. "How agentic AI is driving the next evolution of enterprise AI"
"""

# ╔═╡ f4c27df9-bbc1-4498-b7a0-42da0d049199
const URL = "https://www.moveworks.com/us/en/resources/blog/agentic-ai-the-next-evolution-of-enterprise-ai"
...
Important:
- main concepts, definitions and examples should be presented as itemized list, highlighting their key properties.
...
- metadata about the article such author name(s), date of publication/revision, summary if given... should be placed in the first section of your synthesis 
...
# ╔═╡ 41ac17d0-64d5-46b5-88e3-68eb8eb432f9
# parsed_doc.root[1]  # parsed_doc has two children

# ╔═╡ 1552f513-758b-4f25-bfa2-b4a8f190106d
# parsed_doc.root[2].children    # parsed_doc.root[2] is the root of interest

# ╔═╡ f10f76bc-f1c5-4335-851c-ddeb90ddbd83
# parsed_doc.root[2].children[2]  # article's title and content

# ╔═╡ 78389e7c-e6a5-4bd2-9274-d1a2c68a4920
rroot = parsed_doc.root[2].children[2].children[1];

# ╔═╡ f496d90f-b390-442e-b809-86e4c19f34e6
rroot |> propertynames, tag(rroot.parent)

# ╔═╡ a30efb0b-6fd2-4a57-9a95-8aeb37d73d9c
# rroot
...
text = extract_content( 
	rroot;
	selectors=[".content-well"],  # ["div.ch.bg.fy.fz.ga.gb"], p.pw-post-body-paragraph"],
	verbose=true,
	only_tags=[:div, :h2, :h3, :p, :b, :ul, :li]
	# [:div, :p, :h1, :h2, :h3, :li, :ul, :ol, :span, :pre]  # include :pre for code snipet
)
...
md_title = extract_content(
	rroot;
	selectors=[".hero.teaser"],  # [".pw-post-title"],
	verbose=false,
	only_tags=[:div, :h1]
)  # Title
...
md = extract_content(
	rroot; 
	selectors=[".hero.teaser"], 
	verbose=false,
	only_tags=[:div, :h2, :h3, :p]
)  # other metadata
...
links = extract_links(
	...
	selectors=["a"], 
	verbose=false, 
	restrict_to=["Moveworks"],
	excluding_regex=r"management|sales|finance|employee|legal|privacy|marketing|product|support|economic|enterprise|help[-_]?desk"i,
  	filtering_regex=r"^(?:layout_nav|global_nav|topnav|post_header|_footer|Follow|Sign in|[a-f0-9]+|/[a-z0-9]*)$"i, 
)
