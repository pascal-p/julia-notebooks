...
# ╔═╡ 629b0c3e-8793-11eb-1d0e-136f579d7e29
...
md"""
(a) Construct a bounded range `r1` for the `evo_tree_booster` parameter `max_depth`, varying between 1 and 12.

 (b) For the `nbins` parameter of the `EvoTreeRegressor`, define the range
"""

# ╔═╡ 1a7e108c-8797-11eb-11f8-f7be28d15ab9
rh = range(model₂, :(evo_tree_regressor.nbins), 
	lower = 2.5, upper= 7.5, 
	scale=x -> 2^round(Int, x))

# ╔═╡ 442fd226-8797-11eb-1112-29004fd41659
md"""
Notice that in this case we've specified a *function* instead of a canned scale, like `:log10`. In this case the `scale` function is applied after sampling (uniformly) between the limits of `lower` and `upper`. Perhaps you can guess the outputs of the following lines of code?
"""

# ╔═╡ 441603aa-8797-11eb-3d0e-c5dc3b62e842
begin
	rh_sampler = sampler(rh, Distributions.Uniform)	
	samples = rand(rh_sampler, 1000);
	histogram(samples, nbins=50, legend=false)
end

# ╔═╡ 43fce00a-8797-11eb-0cee-e584618e1c7a
sort(unique(samples))
