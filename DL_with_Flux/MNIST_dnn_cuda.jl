...
# ╔═╡ 3b769e3c-bd1e-47f9-9646-45d13f77249b
using PlutoUI

# ╔═╡ abee26de-95a9-4d5e-bf4d-7f79e21cf84c
begin
...
# ╔═╡ 986ca9da-e981-4542-aef8-2dc2f13d398c
md"""
...
$(html"<div><sub>&copy; Pascal, Mar 2022</sub></div>")
"""

# ╔═╡ a715f66c-d4d0-4a65-8807-853ac1ebac25
PlutoUI.TableOfContents(indent=true, depth=4, aside=true)

# ╔═╡ 1c3d1e18-2ea6-4ec2-9354-e38f0f14dc07
# using Pkg; Pkg.activate("."); Pkg.instantiate()

# ╔═╡ 341e9455-ae8c-489b-a499-1e65c355f25f
const USE_CUDA = true

# ╔═╡ cf0513d9-3d22-4e28-8fed-f5fb562e8c8d
md"""
#### Load train and test data
"""

# ╔═╡ 0f1918f3-6c6f-47c2-b5f1-8e3147068912
begin
...
# ╔═╡ 90049cb6-de4c-4a02-ab8e-95ac23369825
Random.seed!(42)

# ╔═╡ 362d6617-b785-41bd-8cbd-3dccac490d31
md"""
Let's check an image, randomly picked in the train set.
"""

# ╔═╡ 2913b08f-eb60-40e7-9258-eae5f735b12b
ix = Random.rand(1:size(train_x)[3])

# ╔═╡ dfe2bd6f-f3e2-4fb2-a71f-d222c8690d03
begin
	img = train_x[:, :, ix]
	colorview(Gray, img')
end

# ╔═╡ ff9142bc-ea46-41d1-98f6-36a73682d3df
md"""
Let's check the label for this image.
"""

# ╔═╡ c217f2f0-338a-449d-8750-39f7207bd7d4
train_y[ix]

# ╔═╡ 1e486e52-972e-4d6e-b6e4-e397b55b1130
md"""
#### Prepare the data
"""

# ╔═╡ 51c60101-3473-4a17-bc0d-d48834da9f17
md"""
As we are going to use a simple ANN for classifying the digits, we need to turn this gray images (of shape 28 x 28 x 1 - 1 being for the unique channel) into a vector of length 28 × 28 = 784.
...
# ╔═╡ 9afc889d-c87d-45fd-a3f5-13533449162f
xtrain, xtest = Flux.flatten(train_x), Flux.flatten(test_x)

# ╔═╡ a60c60c4-db7f-473b-88d7-211a776ec4d6
md"""
For the labels we need to onehotencode them. This will be useful during the training stage to evaluate the loss function.

This needs to be apllied on both the train and test labels.
"""

# ╔═╡ de57ae3a-ef6f-4997-8b9d-d8a40839ab75
ytrain, ytest = Flux.onehotbatch(train_y, 0:9), Flux.onehotbatch(test_y, 0:9)

# ╔═╡ cd49d4a1-02f2-41d9-9d5a-5019015cd50a
const BATCH_SIZE = 32

# ╔═╡ d71757a9-21f3-4bbc-84f6-fa81331184bc
begin
	train_loader = Flux.Data.DataLoader((xtrain, ytrain), batchsize=BATCH_SIZE, shuffle=true)
	test_loader = Flux.Data.DataLoader((xtest, ytest), batchsize=BATCH_SIZE)
end

# ╔═╡ 695bf8ac-9fe9-4ae7-8c71-ec3b0167063c
md"""
#### Defining the model
"""

# ╔═╡ d62925eb-bdde-49be-97e5-f8f775baa63d
const N_UNITS = [48, 10]  # 48 units in the 1st hidden layer and 10 in the output layer

# ╔═╡ 1f3634d7-a7fb-443b-957d-6ce68cbeaf2e
if CUDA.functional() && USE_CUDA
	@info "Training using CUDA GPU"
...
# ╔═╡ 8651e8af-db83-4825-a85f-e9ad45663bc7
w, h = size(train_x)[1:2]

# ╔═╡ 945b720a-59a1-4868-86ad-e1aecfc752aa
model = Flux.Chain(
	Flux.Dense(w * h, N_UNITS[1], Flux.relu),
	Flux.Dense(N_UNITS...),  # no activation, ouptut the logit...
	# Flux.softmax             # and finally  apply the softnax activation
) |> device

# ╔═╡ 17b84a0d-a48c-4911-9129-2554c280e19a
md"""
###### Define the loss
"""

# ╔═╡ 288fc72b-82b7-4c86-8389-46a3b6db6d83
function loss_acc(model, data_loader, device)
	loss, acc = 0.0, 0.0
	n = 0
	for (x, y) ∈ data_loader
...
# ╔═╡ 9bca5e12-7b32-4402-a8e8-1915e4f17782
md"""
###### Get the parameters of the model
"""

# ╔═╡ 46330d4e-71de-4e23-8ec9-a478882c32dc
prms = Flux.params(model);  # The parameters of the model ≡ 2 weights matrices and 2 bias vectors

# ╔═╡ ca747ed4-d936-4559-9f6b-9e69a906d5aa
# prms[1] |> size, extrema(prms[1])  # weight matrix for 1st layer 

# ╔═╡ 7def127d-f065-4f18-9645-a02cc5ed7d02
# prms[2] |> size  # the bias vector for the 1st layer

# ╔═╡ 480d0a42-1b5d-4162-bdcf-8c4de8c1f6ed
# prms[3] |> size, extrema(prms[3]) # the weigth matrix for the 2nd (output) layer

# ╔═╡ 30729f9b-d373-4524-9c52-a780632dffb3
# prms[4] |> size  # the bias vector for the 2nd (output and last) layer

# ╔═╡ dc6b2ef4-ab7d-4e95-b293-658592bee977
md"""
###### Define the optimizer
"""

# ╔═╡ 04a8eb3f-0e17-43be-850b-6a990381b4aa
begin
	η = 0.01
	opt = Flux.ADAM(η)
end

# ╔═╡ 0a5b6611-5f31-4578-b67a-32eafe0b7802
md"""
#### Training the model
"""

# ╔═╡ f7319f3b-4021-4c2d-a39c-7d2e43fd2556
begin
	const EPOCHS = 100
	loss_train_hist, loss_test_hist = fill(zero(Float32), EPOCHS), fill(zero(Float32), EPOCHS)
	acc_train_hist, acc_test_hist = fill(zero(Float32), EPOCHS), fill(zero(Float32), EPOCHS)
end

# ╔═╡ a12d413d-831a-4f83-92a0-bf9b97f3cda1
 for epoch ∈ 1:EPOCHS
	for (x, y) ∈ train_loader
		x, y = device(x), device(y)
		grads = Flux.gradient(() -> Flux.Losses.logitcrossentropy(model(x), y), prms)
		Flux.Optimise.update!(opt, prms, grads)
	end
	
	loss_train_hist[epoch], acc_train_hist[epoch] = loss_acc(model, train_loader, device)
	loss_test_hist[epoch], acc_test_hist[epoch] = loss_acc(model, test_loader, device)

	if epoch % 50 == 0 
		print("Epoch: $(epoch)") 
		println("- train loss: $(loss_train_hist[epoch]) train acc: $(acc_train_hist[epoch]) - test loss: $(loss_test_hist[epoch]) test acc: $(acc_test_hist[epoch])")
	end
end 

# ╔═╡ 7707c282-6987-49b1-83a1-dcab0f1b9ea9
md"""
###### Let's determine the accuracy of this model
"""

# ╔═╡ 47f8eb9b-79d7-4abc-bcac-1ff9ea8866a8
# begin
# 	ŷ_raw = model(xtest)
# 	ŷ = Flux.onecold(ŷ_raw) .- 1 
# end

# ╔═╡ f94d117b-7604-4cdc-b931-38633edca8b8
# begin
# 	y = Flux.onecold(ytest) .- 1
# 	Statistics.mean(ŷ .== y)
# end
# 0.9608 with 32 units in hidden layer
# 0.9681 with 48 units  "   "     "

# ╔═╡ c8c442da-14ad-4f26-8ad8-9c56a75521d7
# display some results
# check = ŷ .== y

# ╔═╡ 563881f6-13e5-436c-addf-f0dbf7e7fb33
# TODO: check errors

# ╔═╡ 35260335-a832-469e-be1b-a5895addc1ef
md"""
###### Let's plot the loss (during the training)
"""

# ╔═╡ 8cf751f8-2087-4eaf-9815-522c4d202226
begin
	gr(size = (600, 400))
	loss_curve = plot(
		1:EPOCHS,
...
# ╔═╡ f0397a7b-74ad-4e4d-9e49-d90dcad9134e
html"""
<style>
  main {
