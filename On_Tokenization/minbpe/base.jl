...
"""
train(::Tokenizer, _text::String, _vocab_size::Int, _verbose=false) = throw(ArgumentError("Not implemented"))

"""
Tokenizer can encode a string into a list of integers
"""
encode(::Tokenizer, _text::String)::Vector{<: Integer} = throw(ArgumentError("Not implemented"))

"""
Tokenizer can decode a list of integers into a string
"""
decode(::Tokenizer, _ids::Vector{UInt8})::String = throw(ArgumentError("Not implemented"))
...
"""
function save(self::Tokenizer, file_prefix::String)::Nothing
  model_file = string(file_prefix, ".model")

  open(model_file, "w") do f
    # write the version, pattern, and merges
    write(f, "minbpe v1\n")
    write(f, "$(pattern)\n")  # Assuming `pattern` is available in the current scope
    # write the special tokens, first the number of them, then each one
    @printf(f, "%d\n", length(self.special_tokens))
    for (special, idx) ∈ special_tokens
      write(f, "$special $idx\n")
    end
    # the merges dict
    for (idx1, idx2) ∈self.merges
      write(f, "$idx1 $idx2\n")
    end
  end

  vocab_file = string(file_prefix, ".vocab")
  inverted_merges = Dict(idx => pair for (pair, idx) ∈ self.merges)
  open(vocab_file, "w") do f  # , enc="utf-8"
    for (idx, token) ∈ self.vocab
...
"""
function load(self::Tokenizer, model_file::String)::Nothing
  ...
  self = Tokenizer(merges=merges, pattern=self.pattern, special_tokens=special_tokens)
end
